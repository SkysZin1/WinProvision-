...
Native Windows desktop application (C# / WPF) for creating a personalized Windows installation USB. Version 0.5.0 is the current release, with the activation option and the USB writer included.
...
- Review, backup acknowledgment and a typed `ERASE DISK n` confirmation.
...
- English app picker at first sign-in: 30 optional apps, search, visible category sections, per-app progress, retry, logs and postponement. Nothing is selected automatically. Use Applications in the Builder to choose apps now and authorize automatic installation at first sign-in, or choose after first boot and click Install there.
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tests\Writer-Guards.Tests.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tests\Writer-Layout.Tests.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File .\Tests\Apps-Worker.Tests.ps1
powershell -NoProfile -STA -ExecutionPolicy Bypass -File .\Scripts\Provisioner.ps1 -ValidateUi
```

Self-tests check generated XML escaping, credential omission, OS filtering, account validation, and absence of target-disk automation and app installation during specialize. Guard tests mock disk cmdlets and check rejection of system disks, non-USB disks, changed devices, source files on the destination, and missing ISOs. Neither test writes a physical USB. Generated test files contain synthetic credentials only.
