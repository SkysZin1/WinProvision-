...
Native Windows desktop application (C# / WPF) for creating a personalized Windows installation USB. Version 0.6.0 adds selectable Brazilian Portuguese and English interfaces, recommended app removals and more optional removal choices. Real installation/boot testing is still required before a stable release.

## Open the app

Download and extract **WinProvision-desktop-v0.6.0.zip** from [Releases](https://github.com/SkysZin1/WinProvision-/releases/tag/v0.6.0), then open `WinProvision.exe`. The repository copy is under `packaging\releases\WinProvision-desktop-v0.6.0`. After building from source, use `src\WinProvision\bin\Release\net8.0-windows\WinProvision.exe`. Keep the executable, DLLs, runtime configuration, `Assets` and `Scripts` folders together. The Builder requires the **.NET 8 Desktop Runtime**. The installed target Windows uses built-in Windows PowerShell/WPF and does not need .NET 8.
...
- Bilingual app picker at first sign-in: 30 optional apps, search, categories, per-app progress, retry, logs and postponement. Use Applications in the Builder to select and authorize apps before installation, or choose after first boot. The standalone picker preview button has been removed; application selection remains available.
