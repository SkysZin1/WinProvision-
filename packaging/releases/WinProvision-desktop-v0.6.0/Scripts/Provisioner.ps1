...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml" Title="{DynamicResource Picker001}" Width="1080" Height="800" MinWidth="800" MinHeight="640" WindowStartupLocation="CenterScreen" Background="#F3F6F5" FontFamily="Segoe UI" FontSize="14" Foreground="#172A34">
 ...
 <Grid Margin="30,24">
  ...
  <StackPanel Grid.Row="4" Margin="0,14,0,12"><TextBlock Name="Status" Text="{DynamicResource Picker011}" FontWeight="SemiBold"/><ProgressBar Name="Progress" Height="5" Margin="0,9,0,8" Foreground="#0B7467"/><TextBlock Name="InstallTerms" Text="{DynamicResource Picker012}" Foreground="#607480" FontSize="12"/></StackPanel>
...
foreach($name in @('Apps','Search','Category','Environment','Status','Progress','Install','Later','Logs','Stop','Store','InstallTerms','LanguageChoice')) { Set-Variable -Name $name -Value $window.FindName($name) }
...
function Update-PickerLanguage {
 ...
 $Status.Text=(TF '{0} app(s) selected.' @($count))
}
...
try {
 if($ValidateUi){
  ...
  $Search.Text='';$Category.SelectedItem=(T 'Development');if(@($view).Count -ne 7){throw 'Category filter failed.'}
