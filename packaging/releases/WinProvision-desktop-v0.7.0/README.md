...
Version **0.7.0** includes 36 apps, installation summaries, retrying only failed apps, and internet/App Installer readiness checks.

Download and extract **WinProvision-desktop-v0.7.0.zip** from [Releases](https://github.com/SkysZin1/WinProvision-/releases/tag/v0.7.0), then open `WinProvision.exe`. The repository copy is under `packaging\releases\WinProvision-desktop-v0.7.0`. After building from source, use `src\WinProvision\bin\Release\net8.0-windows\WinProvision.exe`. Keep the executable, DLLs, runtime configuration, `Assets` and `Scripts` folders together. The Builder requires the **.NET 8 Desktop Runtime**. The installed target Windows uses built-in Windows PowerShell/WPF and does not need .NET 8.
