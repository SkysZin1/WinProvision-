...
public static class Catalog
{
 ...
 [
  ...
  new("activate-windows", "Setup", "Activate Windows", "Show an activation confirmation window alongside App Picker and add a desktop shortcut. Internet is required; the terminal opens only after confirmation.", Both, "Xml", ""),
  ...
  new("remote-signed", "System", "Allow local PowerShell scripts", "Set LocalMachine execution policy to RemoteSigned. Domain policy can override this setting.", Both, "System", "try { Set-ExecutionPolicy -Scope LocalMachine -ExecutionPolicy RemoteSigned -Force } catch { if((Get-ExecutionPolicy -Scope LocalMachine) -ne 'RemoteSigned') { throw } }", true),
