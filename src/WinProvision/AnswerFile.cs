 ...
 public static string Generate(BuildProfile p, string password, bool preview = false)
 {
  ...
  provisioner=provisioner.Replace("$initialPlan=@{Mode='AfterBoot';Ids=@()}","$initialPlan=ConvertFrom-Json @'\r\n"+appPlan+"\r\n'@");
  {
   var extract = """
    ...
    foreach($entry in @(@('Script','Configure.ps1'),@('Provisioner','Provisioner.ps1'))) {
  ...
  if (oobeOptions.HasElements) accountShell.Add(oobeOptions);
  {
   ...
   accountShell.Add(E("FirstLogonCommands", E("SynchronousCommand", new object[] { new XAttribute(Wcm+"action","add"), E("Order",1), E("Description","Clean up WinProvision setup files"), E("CommandLine", "powershell.exe -NoProfile -ExecutionPolicy Bypass -EncodedCommand " + Convert.ToBase64String(Encoding.Unicode.GetBytes(clean))) })));
  }
  if (accountShell.HasElements) oobe.Add(accountShell);
  if (oobe.HasElements) root.Add(oobe);
  root.Add(new XElement(Ext+"Extensions", scripts == null ? null : new XElement(Ext+"Script", scripts), new XElement(Ext+"Provisioner", provisioner)));
