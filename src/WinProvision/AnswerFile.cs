 ...
 public static string Generate(BuildProfile p, string password, bool preview = false)
 {
  ...
  provisioner=provisioner.Replace("$initialPlan=@{Mode='AfterBoot';Ids=@()}","$initialPlan=ConvertFrom-Json @'\r\n"+appPlan+"\r\n'@");
  {
   var extract = """
    ...
    foreach($entry in @(@('Script','Configure.ps1'),@('Provisioner','Provisioner.ps1'),@('Localization','Translations.json'),@('LocalizationScript','Localization.ps1'),@('ActivateWindows','activateWindows.cmd'))) {
  ...
  root.Add(new XElement(Ext+"Extensions",
    ...
    new XElement(Ext+"Localization", L.Json),
    new XElement(Ext+"LocalizationScript", localizationReader.ReadToEnd()),
