...
public sealed class ApplicationsWindow : Window
{
 ...
 {
  Result=source.Clone();Title="Applications";Width=640;Height=480;WindowStartupLocation=WindowStartupLocation.CenterOwner;
  var panel=new StackPanel{Margin=new Thickness(28)};Content=panel;
  panel.Children.Add(new TextBlock{Text="When should apps be selected?",FontSize=24,FontWeight=FontWeights.SemiBold,Margin=new Thickness(0,0,0,22)});
  var later=new RadioButton{Content="Choose after first boot",IsChecked=Result.AppSelectionMode=="AfterBoot",Margin=new Thickness(0,0,0,14)};
  var now=new RadioButton{Content="Choose now — install automatically after first sign-in",IsChecked=Result.AppSelectionMode=="BeforeBoot",Margin=new Thickness(0,0,0,18)};
  panel.Children.Add(later);panel.Children.Add(now);
  var choose=new Button{Content="Choose apps…",HorizontalAlignment=HorizontalAlignment.Left,IsEnabled=now.IsChecked==true};panel.Children.Add(choose);
  ...
  panel.Children.Add(new TextBlock{Text="Choose now authorizes installation of the selected apps and accepts their terms and the WinGet source terms. Downloads start on the installed Windows, never on this PC. Internet is required; permission prompts or failures may need attention. Use Recommended for automatic Windows setup and sign-in.",FontSize=12,Margin=new Thickness(0,0,0,18)});
  var save=new Button{Content="Save application settings",HorizontalAlignment=HorizontalAlignment.Right};panel.Children.Add(save);
  ...
  choose.Click+=async(_,_)=>{
   ...
   try{
    ...
    foreach(var arg in new[]{"-NoProfile","-STA","-ExecutionPolicy","Bypass","-File",Path.Combine(AppContext.BaseDirectory,"Scripts","Provisioner.ps1"),"-Configure","-SelectionPath",selection})start.ArgumentList.Add(arg);
    using var process=Process.Start(start)??throw new IOException("Cannot open app selection.");await process.WaitForExitAsync();
    ...
    if(response.RootElement.GetProperty("Saved").GetBoolean()){
     ...
     if(ids.Any(id=>!AppCatalog.Names.ContainsKey(id)))throw new InvalidDataException("Unknown app selection.");
     Result.SelectedApps=ids;summary.Text=string.Join(", ",ids.Select(id=>AppCatalog.Names[id]));
    }
   }catch(Exception ex){MessageBox.Show(this,ex.Message,"Applications");}
   finally{choosing=false;choose.IsEnabled=save.IsEnabled=later.IsEnabled=now.IsEnabled=true;}
  };
  save.Click+=(_,_)=>{if(Result.AppSelectionMode=="BeforeBoot"&&Result.SelectedApps.Count==0){MessageBox.Show(this,"Choose at least one app.");return;}DialogResult=true;};
 }
}
