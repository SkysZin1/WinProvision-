...
if($Worker) {
 ...
 try {
  ...
  try {
   ...
   $_ | Out-String | Set-Content -LiteralPath (Join-Path $JobPath 'connectivity.log')
   try {
    $connection=Invoke-WebRequest -Uri 'https://cdn.winget.microsoft.com/cache/source.msix' -Method Head -UseBasicParsing -TimeoutSec 15 -ErrorAction Stop
    if($connection.StatusCode -ne 200){throw 'Download service is unavailable.'}
   } catch {
    $_ | Out-String | Add-Content -LiteralPath (Join-Path $JobPath 'connectivity.log')
    throw (T 'Internet access could not be verified. Connect to Wi-Fi or Ethernet, complete any network sign-in, then retry. No apps were installed.')
   }
  }
  ...
  if($needsWinget){
   try {
    ...
    try {
     # PowerShell 5.1 can otherwise lose the exit code after WaitForExit closes its temporary handle.
     $null=$probe.Handle
    ...
    $_ | Out-String | Set-Content -LiteralPath (Join-Path $JobPath 'winget-probe.log')
    throw (T 'App Installer is not responding correctly. Open Microsoft Store, install or update App Installer, then retry. No apps were installed.')
   }
  }
